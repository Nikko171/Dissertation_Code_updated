...
lc1 = 'Data/Fiducial/z05/3650_scaled_light_curve.csv'
lc2 = 'Data/Fiducial/z05/4640_scaled_light_curve.csv'
lc_3650 = pd.read_csv(lc1)
lc_4640 = pd.read_csv(lc2)
lower_clip = 100
upper_clip = 17233
time_step = 40 
mjd1 = lc_3650['time'][lower_clip:upper_clip:time_step].values
print(type(mjd1), len(mjd1))
flux1 =  lc_3650['flux [mJy]'][lower_clip:upper_clip:time_step].values
err1 = lc_3650['error'][lower_clip:upper_clip:time_step].values
mjd2 = lc_4640['time'][lower_clip:upper_clip:time_step].values
flux2 =  lc_4640['flux [mJy]'][lower_clip:upper_clip:time_step].values
err2 = lc_4640['error'][lower_clip:upper_clip:time_step].values

#########################################
##Set Interpolation settings, user-specified
#########################################
lag_range = [-50, 50]  #Time lag range to consider in the CCF (days). Must be small enough that there is some overlap between light curves at that shift (i.e., if the light curves span 80 days, these values must be less than 80 days)
...
plt.savefig(output_file_folder+'sample_results_plot.png', format = 'png', orientation = 'landscape', bbox_inches = 'tight') 
plt.close(fig)