...
import stark_temp_profile as mt0
...
def pytfb_sub(taus,embh,emdot,wavang, deginc, t0vin=-1, t0iin = -1, alpha_visc = -0.75, 
              hxsch = 3.0, alpha_irad = -0.75, eta = 0.1, rlosch = 3.0, norm = True, 
              quick = 1, xstop = 15, udlnr = 0.01,thcent=1.0,thfwhm=0.2, oldsmooth = False,
              newsmooth = True, diagnose=False):
    '''
    Transfer Function of an accretion disc
    Adapted from David Starkey PyCREAM (Starkey et al. 2016, MNRAS)
    
	taus : array
		Time array to evaluate the Transfer function. Units assumed
        to be in days.
    embh : float
        Black Hole mass. M_sol
    emdot : float
        Mass accretion rate in M_sol/yr
    waveang : float
        Central wavelength to calculate the delay transfer. Angstrom
    deginc : float
        Inclination angle in degrees.

	t0vin : float, optional
        Reference Temperature of the disc at 1 light day. By default, it will be
        calculated using the emdot and embh values. Default: -1
    t0iin : float, optional
        Reference Temperature of the disc. By default, it will be
        calculated using the emdot and embh values. Default: -1
    alpha_visc : float, optional
        Temperature-Radius profile index (viscous heating), T(R) \proptp R^{alpha_visc} 
        Default: -0.75
    hxscha : float, optional
        Corona height (source of irradiation) in Gravitational radii R_g
        Default: 3.0
    alpha_irad : float, optional
        Temperature-Radius profile index  of the irradiated disc, T(R) \proptp R^{alpha_irad} 
        Default: -0.75
    eta : float, optional
        Accretion efficiency. Default: 0.1
    '''
    tauhi  = taus[-1]
    taulo  = taus[0]
    ...
    if (wavang < 0.0):
        ...
        #raw_input()
        #build grid
        rgridlog = np.exp( np.arange(np.log(rlold), np.log(rhilog), udlnr)[:-1] )
  ...
  #rgrid  = np.logspace(np.log10(rlold),np.log10(rhild),nrad)
        ...
        if (quick == 1):
            for i in range(nrad-1):
                ...
                az   = np.random.uniform(low=azgrid[:-1],high=azgrid[1:],size=nazsub1)
                #np.random.uniform(low=0,high=1,size=nazsub1)*azgrid_s + 
                #azgrid[:-1]#np.random.uniform(azgrid[:-1],azgrid[1:],1)
                caz  = np.cos(az)
                ...
                weight = rir0b[i]/ttot3now * x2/wavang2/(np.cosh(x) - 1) * radlo*drad*azwidth            
            ...
            for i in range(nrad-1):
                ...
                caz  = np.cos(az)
                tdl  = hxci - raz*caz*sini + daz
                #modification for inner radius to the right
                #(negligible)* (1 - np.sqrt(rlold/raz)) / (1 - np.sqrt(rlold/1))
                #tv4   = t0v4*(raz)**av4 
                ti4   = t0i4*(raz)**ai4
                rir0b = ti4/t0i4 #this is just (r/r0)^alpha_irad
                ttot4 = tv4 + ti4
   ...
   #psis_2 = scipy.ndimage.filters.gaussian_filter1d(, sigtau, axis=-1, order=0, output=None, mode='reflect', cval=0.0, truncate=4.0) 
        ...
        for id in range(nds):
            ...
            #print 'dfsdf',ntaus, np.ceil((delnow + sigtaulim)/dtau), 
            #np.max(ntaus,np.ceil((delnow + sigtaulim)/dtau))
        ...
        if diagnose:
            ...
            rsave = rsave[ida]   
  #print(psis)
  #psis = np.nan_to_num((np.array(psis),0)) 
  #psis = psis[0] 
...
for wav in redshift_wav05:
    psis = pytfb_sub(taus,embhref,emdotref,wav, degref, norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    #psis = pytfb_sub(taus,embhref,emdotref,wav, degref, t0vin=t0v, t0iin=t0i, norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    mean = np.sum(taus*psis)/np.sum(psis)
    print(wav, mean)
    with open( 'Data/Fiducial/z05/' +str(wav)+'_transfer_function.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['taus', 'psis'])  # Write the header
        for tau, psi in zip(taus, psis):
            csvwriter.writerow([tau, psi])
for wav in redshift_wav1:
    #psis = pytfb_sub(taus,embhref,emdotref,wav, degref, norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    psis = pytfb_sub(taus,embhref,emdotref,wav, degref, t0vin=t0v, t0iin=t0i, norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    mean = np.sum(taus*psis)/np.sum(psis)
    print(wav, mean)
    with open( 'Data/Fiducial/z1/' +str(wav)+'_transfer_function.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['taus', 'psis'])  # Write the header
        for tau, psi in zip(taus, psis):
            csvwriter.writerow([tau, psi])
for wav in redshift_wav2:
    #psis = pytfb_sub(taus,embhref,emdotref,wav, degref, t0vin=t0v, t0iin=t0i, norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    psis = pytfb_sub(taus,embhref,emdotref,wav, degref,  norm=0, quick=1, oldsmooth=0, newsmooth=1, diagnose=0)
    mean = np.sum(taus*psis)/np.sum(psis)
    print(wav, mean)
    with open( 'Data/Fiducial/z2/' +str(wav)+'_transfer_function.csv', 'w', newline='') as csvfile:
        csvwriter = csv.writer(csvfile)
        csvwriter.writerow(['taus', 'psis'])  # Write the header
        for tau, psi in zip(taus, psis):
            csvwriter.writerow([tau, psi])
...
for dnow in deg:
    ...
    ax1.plot(taus,psis,color=color[idx])
    #calculate the mean
    mean = np.sum(taus*psis)/np.sum(psis)
